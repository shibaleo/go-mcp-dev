...
func LogToolCall(module, tool string, durationMs int64, status string, errMsg string) {
	...
	Push(labels, data)
	log.Printf("Tool call: module=%s tool=%s duration=%dms status=%s", module, tool, durationMs, status)
}
